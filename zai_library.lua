-- zai: mobile supported ui library
local function zaiShowLoadingNotification()
    local RunService = game:GetService("RunService")
    local guiParent = (gethui and gethui()) or game:GetService("CoreGui")
    local gui = Instance.new("ScreenGui")
    gui.Name = "zaiLoadingNotification"
    ...
    label.Size = UDim2.fromOffset(math.floor(280 * Library.MobileScale), math.floor(48 * Library.MobileScale))
...
getgenv().zaiModuleCache = getgenv().zaiModuleCache or {}

local function zaiSafeRequire(moduleRef, timeoutSec)
    local cache = getgenv().zaiModuleCache
    ...
    error("[zai] module load failed: " .. key .. " (" .. tostring(lastErr) .. ")")
end

getgenv().zaiRequire = zaiSafeRequire

local function zaiPreloadCoreModules()
    ...
    pcall(function() zaiSafeRequire(modulesFolder:WaitForChild("EnumLibrary", 4)) end)
    pcall(function() zaiSafeRequire(modulesFolder:WaitForChild("Utility", 4)) end)
    local controllers = playerScripts:WaitForChild("Controllers", 8)
    pcall(function() zaiSafeRequire(controllers:WaitForChild("FighterController", 4)) end)
    pcall(function() zaiSafeRequire(controllers:WaitForChild("CameraController", 4)) end)
end

local function zaiHideErrorPromptsStep()
...
local function zaiRunLoadingBootstrap()
    local runService = game:GetService("RunService")
    local dismissLoading = zaiShowLoadingNotification()
    local hideErrors = true
    local hideConn = runService.RenderStepped:Connect(function()
        if hideErrors then zaiHideErrorPromptsStep() end
    end)

    task.wait(0.8)

    pcall(zaiPreloadCoreModules)
...
local function zaiPlainReplace(str, find, replace)
    local startPos = 1
    while true do
...
getgenv().zaiSliderFillState = getgenv().zaiSliderFillState or {}
getgenv().zaiSliderFollowRate = getgenv().zaiSliderFollowRate or 11

getgenv().zaiSetSliderFill = function(fill, targetX, hideBorder, sliderObj, forceSnap)
    if not fill or not fill.Parent then return end
    targetX = math.floor(tonumber(targetX) or 0)
    local stateMap = getgenv().zaiSliderFillState
    if forceSnap then
...
if not getgenv().zaiSliderFillBound then
    getgenv().zaiSliderFillBound = true
    RunService.RenderStepped:Connect(function(dt)
        local stateMap = getgenv().zaiSliderFillState
        if not stateMap then return end
        dt = math.clamp(tonumber(dt) or 0, 1 / 1000, 1 / 15)
        local follow = getgenv().zaiSliderFollowRate or 11
...
local function loadZaiLibrary()
    local cacheKey = "zaiLibrary_Cached_Patched"
    if getgenv()[cacheKey] then return getgenv()[cacheKey]() end
    local rawSrc
    if isfile and isfile("zaiLibrary_Patched.lua") then
        rawSrc = readfile("zaiLibrary_Patched.lua")
    ...
    if not rawSrc then
        if isfile and isfile("zaiLibrary_Patched.lua") then
            pcall(delfile, "zaiLibrary_Patched.lua")
        end
...
Library.MobileScale = IsMobile and 0.75 or 1
...
local DisplayFrame = Library:Create('Frame', {
    ...
    BorderColor3 = Library.OutlineColor;
    Size = UDim2.new(0, math.floor(28 * Library.MobileScale), 0, math.floor(14 * Library.MobileScale));
    ZIndex = 9;
    Parent = ToggleLabel;
});
...
local DisplayInner = Library:Create('Frame', {
    ...
    Parent = DisplayFrame;
        ...
        local PickerFrameOuter = Library:Create('Frame', {
            ...
            Size = UDim2.fromOffset(math.floor(230 * Library.MobileScale), math.floor(280 * Library.MobileScale));
            Visible = false;
            ZIndex = 15;
            Parent = ScreenGui,
        Active = true,
        ...
        local SatVibMapOuter = Library:Create('Frame', {
            ...
            Size = UDim2.new(0, math.floor(200 * Library.MobileScale), 0, math.floor(200 * Library.MobileScale));
            ZIndex = 17;
            Parent = PickerFrameInner;
        });
        ...
        local CursorOuter = Library:Create('ImageLabel', {
            ...
            Size = UDim2.new(0, math.floor(6 * Library.MobileScale), 0, math.floor(6 * Library.MobileScale));
            BackgroundTransparency = 1;
        ...
        local HueSelectorOuter = Library:Create('Frame', {
            ...
            Size = UDim2.new(0, math.floor(15 * Library.MobileScale), 0, math.floor(200 * Library.MobileScale));
            ZIndex = 17;
            Parent = PickerFrameInner;
        });
...
local function CreateRGBSlider(Name, YPos, ColorChannel)
    local SliderLabel = Library:CreateLabel({
        ...
        Size = UDim2.new(0, math.floor(15 * Library.MobileScale), 0, math.floor(15 * Library.MobileScale));
        Text = Name;
        TextSize = 12;
        ZIndex = 19;
    ...
    local ValueLabel = Library:CreateLabel({
        ...
        Size = UDim2.new(0, math.floor(25 * Library.MobileScale), 0, math.floor(15 * Library.MobileScale));
        Text = '255';
        TextSize = 12;
        ZIndex = 19;
        Parent = SliderContainer;
...
CreateRGBSlider('B', 40, 'B');
        ...
        TransparencyBoxOuter = Library:Create('Frame', {
            ...
            Size = UDim2.new(0, math.floor(200 * Library.MobileScale), 0, math.floor(15 * Library.MobileScale));
            ZIndex = 19;
            Parent = PickerFrameInner;
        });
...
Library:Create(DisplayInner, {
    ...
    BackgroundTransparency = ColorPicker.Transparency;
        ...
        local PickOuter = Library:Create('Frame', {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Size = UDim2.new(0, math.floor(28 * Library.MobileScale), 0, math.floor(15 * Library.MobileScale));
            ZIndex = 9;
            Parent = ToggleLabel;
        });
        ...
        local ModeSelectOuter = Library:Create('Frame', {
            ...
            Size = UDim2.new(0, math.floor(60 * Library.MobileScale), 0, math.floor(47 * Library.MobileScale));
            Visible = false;
            ZIndex = 14;
            Parent = ScreenGui;
        });
...
function Button:AddButton(...)
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local ToggleOuter = Library:Create('Frame', {
            BackgroundColor3 = Color3.new(0, 0, 0);
            BorderColor3 = Color3.new(0, 0, 0);
            Size = UDim2.new(0, math.floor(13 * Library.MobileScale), 0, math.floor(13 * Library.MobileScale));
            ZIndex = 5;
            Parent = Container;
        });
...
local Slider = {
        ...
        function Slider:Display()
            ...
            getgenv().zaiSetSliderFill(Fill, X, HideBorderRight, Slider);
...
SliderInner.InputBegan:Connect(function(Input)
    if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
        ...
        while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
            ...
            getgenv().zaiSetSliderFill(Fill, nX, HideBorderRight, Slider);
...
local MinSlider = CreateSlider(true, UDim2.new(0, 0, 0, 0));
        ...
        function MultiSlider:Display()
            ...
            getgenv().zaiSetSliderFill(MinSlider.Fill, MinX);
            getgenv().zaiSetSliderFill(MaxSlider.Fill, MaxX);
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    if nValue <= MultiSlider.MaxValue then
                        ...
                        getgenv().zaiSetSliderFill(MinSlider.Fill, nX);
...
MaxSlider.Inner.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    if nValue >= MultiSlider.MinValue then
                        ...
                        getgenv().zaiSetSliderFill(MaxSlider.Fill, nX);
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local DropdownArrow = Library:Create('ImageLabel', {
            ...
            Size = UDim2.new(0, math.floor(12 * Library.MobileScale), 0, math.floor(12 * Library.MobileScale));
...
Library:AddToRegistry(ListInner, {
        ...
        local Scrolling = Library:Create('ScrollingFrame', {
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
            CanvasSize = UDim2.new(0, math.floor(0 * Library.MobileScale), 0, math.floor(0 * Library.MobileScale));
            Size = UDim2.new(1, 0, 1, 0);
            ZIndex = 21;
            Parent = ListInner;
...
do
    Library.NotificationArea = Library:Create('Frame', {
        ...
        Size = UDim2.new(0, math.floor(300 * Library.MobileScale), 0, math.floor(200 * Library.MobileScale));
        ZIndex = 100;
        Parent = ScreenGui;
    });
    ...
    local WatermarkOuter = Library:Create('Frame', {
        ...
        Size = UDim2.new(0, math.floor(213 * Library.MobileScale), 0, math.floor(20 * Library.MobileScale));
        ZIndex = 200;
        Visible = false;
        Parent = ScreenGui;
    });
    ...
    local KeybindOuter = Library:Create('Frame', {
        ...
        Size = UDim2.new(0, math.floor(210 * Library.MobileScale), 0, math.floor(20 * Library.MobileScale));
        Visible = false;
        ZIndex = 100;
        Parent = ScreenGui;
    });
...
function Library:CreateWindow(...)
    ...
    if typeof(Config.Size) ~= 'UDim2' then Config.Size = UDim2.fromOffset(math.floor(700 * Library.MobileScale), math.floor(600 * Library.MobileScale)) end
...
local WindowLabel = Library:CreateLabel({
    ...
    Size = UDim2.new(0, math.floor(300 * Library.MobileScale), 0, math.floor(30 * Library.MobileScale));
...
local TabArea = Library:Create('Frame', {
    ...
    Size = UDim2.new(0, math.floor(310 * Library.MobileScale), 0, math.floor(21 * Library.MobileScale));
    ZIndex = 5;
    Parent = Inner;
});
    ...
    TabButtonLabel.TextColor3 = Color3.fromRGB(150, 150, 150);
        ...
        function Tab:AddGroupbox(Info)
            ...
            local Highlight = Library:Create('Frame', {
                BackgroundTransparency = 1;
                BorderSizePixel = 0;
                Size = UDim2.new(0, math.floor(0 * Library.MobileScale), 0, math.floor(0 * Library.MobileScale));
                Visible = false;
                Parent = BoxInner;
            });
...
return Library]==]
    ...
    local replaces = {
        ["Fill.Size = UDim2.new(0, X, 1, 0);"] = "getgenv().zaiSetSliderFill(Fill, X, HideBorderRight, Slider);",
        ["Fill.Size = UDim2.new(0, nX, 1, 0);"] = "getgenv().zaiSetSliderFill(Fill, nX, HideBorderRight, Slider);",
        ["MinSlider.Fill.Size = UDim2.new(0, MinX, 1, 0);"] = "getgenv().zaiSetSliderFill(MinSlider.Fill, MinX);",
        ["MaxSlider.Fill.Size = UDim2.new(0, MaxX, 1, 0);"] = "getgenv().zaiSetSliderFill(MaxSlider.Fill, MaxX);",
        ["MinSlider.Fill.Size = UDim2.new(0, nX, 1, 0);"] = "getgenv().zaiSetSliderFill(MinSlider.Fill, nX);",
        ["MaxSlider.Fill.Size = UDim2.new(0, nX, 1, 0);"] = "getgenv().zaiSetSliderFill(MaxSlider.Fill, nX);",
    ...
    if writefile then pcall(writefile, "zaiLibrary_Patched.lua", patchedSrc) end
    ...
    error("[zai] library failed to load")
end

local Library = loadZaiLibrary()
...
local function applyZaiAccentTheme()
...
applyZaiAccentTheme()
getgenv().zaiApplyAccentTheme = applyZaiAccentTheme
...
local function applyzaiUiLayering()
    pcall(function()
    ...
    pcall(function()
        local st = getgenv().zaiCosmeticUIState
    ...
    pcall(function()
        local kb = getgenv().zaiKeybindList
...
getgenv().zaiApplyUiLayering = applyzaiUiLayering
...
getgenv().zaiMenuCursor = getgenv().zaiMenuCursor or {}

local function ensurezaiMenuCursor()
    local mc = getgenv().zaiMenuCursor
...
local function shouldSuppressGameplayOverlays()
    return iszaiMenuOpen()
end

ensurezaiMenuCursor()

local MENU_CURSOR_HZ = 240
local MENU_CURSOR_BIND = "zaiMenuCursorDraw"
local menuCursorAccum = 0

local function drawzaiMenuCursor()
    local mc = getgenv().zaiMenuCursor
    if not mc or not mc.cursor or not mc.outline then return end
    local open = iszaiMenuOpen()
    if not open then
...
getgenv().zaiIsMenuOpen = iszaiMenuOpen

getgenv().zaiLanguage = getgenv().zaiLanguage or "english"

local zaiWeaponTranslations = {
    korean = {
...
getgenv().zaiTranslateWeapon = function(name)
    if not name or type(name) ~= "string" then return name end
    local lang = getgenv().zaiLanguage or "english"
    if lang == "english" then return name end
    local pack = zaiWeaponTranslations[lang]
    return (pack and pack[name]) or name
end

getgenv().zaiTranslateLabel = function(text)
    if not text or type(text) ~= "string" then return text end
    local lang = getgenv().zaiLanguage or "english"
    if lang == "english" then return text:lower() end
    local translated = getgenv().zaiTranslateWeapon(text)
    if translated ~= text then return translated:lower() end
    return text:lower()
end

getgenv().zaiGetUiFont = function()
    if getgenv().zaiLanguage == "korean" and getgenv().zaiUIFont then
        return getgenv().zaiUIFont
    end
    if Library and Library.Font then
        return Library.Font
    end
    return getgenv().zaiUIFont or Enum.Font.Roboto
end

getgenv().zaiL = function(key, ...)
    local lang = getgenv().zaiLanguage or "english"
    local pack = zaiLocaleTable[lang] or zaiLocaleTable.english
    local template = pack[key] or zaiLocaleTable.english[key] or key
    if select("#", ...) > 0 then
...
local zaiFontHttp = game:GetService("HttpService")

local zaiFontSources = {
    english = { file = "instance_ui_en.otf", fontFile = "instance_ui_en.font", name = "zaiUI_En", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
    korean = { file = "instance_ui_ko.otf", fontFile = "instance_ui_ko.font", name = "zaiUI_Ko", url = "https://github.com/googlefonts/noto-cjk/raw/main/Sans/OTF/Korean/NotoSansCJKkr-Regular.otf" },
    spanish = { file = "instance_ui_es.otf", fontFile = "instance_ui_es.font", name = "zaiUI_Es", url = "https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf" },
}

local function loadzaiLanguageFont(lang)
    lang = lang or getgenv().zaiLanguage or "english"
    local src = zaiFontSources[lang] or zaiFontSources.english
    local ok, face = pcall(function()
        if isfile and writefile and getcustomasset then
            ...
            writefile(src.fontFile, zaiFontHttp:JSONEncode(fontdata))
            return Font.new(getcustomasset(src.fontFile))
        end
        return nil
    end)
    if ok and face then
        getgenv().zaiUIFont = face
        return face
    end
    getgenv().zaiUIFont = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium)
    return getgenv().zaiUIFont
end

getgenv().zaiReloadLanguageFont = function(lang)
    lang = lang or getgenv().zaiLanguage or "english"
    if lang == "english" and Library and Library.Font then
        getgenv().zaiUIFont = Library.Font
        return Library.Font
    end
    return loadzaiLanguageFont(lang)
end
task.defer(function()
    pcall(function()
        getgenv().zaiReloadLanguageFont(getgenv().zaiLanguage)
    end)
end)

getgenv().zaiDrawingTextPool = getgenv().zaiDrawingTextPool or {}

getgenv().zaiTrackDrawingText = function(obj)
    table.insert(getgenv().zaiDrawingTextPool, obj)
    pcall(function()
        local face = getgenv().zaiUIFont
        if face and obj.FontFace ~= nil then obj.FontFace = face end
    end)
    return obj
end

getgenv().zaiApplyDrawingUIFont = function()
    local face = getgenv().zaiUIFont
    if not face then return end
    for _, obj in ipairs(getgenv().zaiDrawingTextPool) do
        pcall(function()
...
getgenv().zaiApplyUiFont = function(obj, textSize)
    if not obj then return end
    if textSize then obj.TextSize = textSize end
    local font = getgenv().zaiGetUiFont()
    pcall(function()
        if typeof(font) == "Font" then
            obj.FontFace = font
        else
...
getgenv().zaiApplyUIFont = function()
    local face = getgenv().zaiGetUiFont()
    if not face then return end
    getgenv().zaiUIFont = face
    ...
    if getgenv().zaiApplyDrawingUIFont then pcall(getgenv().zaiApplyDrawingUIFont) end
    if getgenv().zaiApplyKbListTheme then pcall(getgenv().zaiApplyKbListTheme) end
end
...
getgenv().zaiHudFonts = {
    ...
    { n = "custom", draw = 2, face = function() return getgenv().zaiUIFont or Font.fromEnum(Enum.Font.Gotham) end },
}
getgenv().zaiHudFontNames = {}
for i, slot in ipairs(getgenv().zaiHudFonts) do
    getgenv().zaiHudFontNames[i] = slot.n
end
getgenv().zaiHudFontIdx = 1
getgenv().zaiHudDrawFont = 2

getgenv().zaiHudFontIndexByName = function(name)
    for i, slot in ipairs(getgenv().zaiHudFonts) do
...
getgenv().zaiApplyHudFont = function(idx)
    idx = math.clamp(tonumber(idx) or getgenv().zaiHudFontIdx or 1, 1, #getgenv().zaiHudFonts)
    local slot = getgenv().zaiHudFonts[idx]
    getgenv().zaiHudFontIdx = idx
    getgenv().zaiHudDrawFont = slot.draw or 2
    local face
    pcall(function() face = slot.face() end)
    if face then getgenv().zaiEspUiFont = face end
    if _G.ESPObjects then
        for _, box in pairs(_G.ESPObjects) do
            if box and box.text then
                for _, lbl in pairs(box.text) do
    ...
    for _, obj in ipairs(getgenv().zaiDrawingTextPool or {}) do
        pcall(function()
            if obj.Font ~= nil then obj.Font = getgenv().zaiHudDrawFont end
            if face and obj.FontFace ~= nil then obj.FontFace = face end
        end)
    end
end
...
local function alignzaiMenuTabs()
...
local UserInputService  = game:GetService("UserInputService")
local Utility           = zaiSafeRequire(ReplicatedStorage.Modules.Utility)
local EnumLibrary       = zaiSafeRequire(ReplicatedStorage.Modules.EnumLibrary)
local LocalPlayer       = Players.LocalPlayer
...
getgenv().zaiGetAspectStretch = getAspectStretch
getgenv().zaiWorldToScreen = worldToScreen
getgenv().zaiScreenAnchor = screenAnchor
getgenv().zaiScreenCenter = screenCenter
getgenv().zaiGetUnstretchedCameraCFrame = getUnstretchedCameraCFrame

_G.Features = _G.Features or {}
_G.Features.CrossbowSoundId = "rbxassetid://165946246"
getgenv().zaiCombatLastShotAt = getgenv().zaiCombatLastShotAt or 0

local function markCombatShot()
    getgenv().zaiCombatLastShotAt = tick()
end
getgenv().zaiMarkCombatShot = markCombatShot

getgenv().zaiWorldToScreenEsp = function(worldPos, cam)
...
local function restoreGunSoundVolumes()
    local function tryRestore(root)
        ...
        for _, d in ipairs(root:GetDescendants()) do
            if d:IsA("Sound") and isRivalsGunSound(d) and not d:GetAttribute("zaiShootSound") then
                pcall(function()
                    if d.Volume <= 0 then
                        d.Volume = 1
                    end
...
local function isRivalsGunSound(sound)
    ...
    if sound:GetAttribute("zaiShootSound") then
        return false
    end
...
local function bindGunSoundMuteGuard(sound)
    if typeof(sound) ~= "Instance" or not sound:IsA("Sound") or sound:GetAttribute("zaiShootSound") then
        return
    end
    if not sound:GetAttribute("zaiGunMuteBound") then
        sound:SetAttribute("zaiGunMuteBound", true)
...
getgenv().zaiRefreshGunSoundMute = refreshGunSoundMute
...
getgenv().zaiHandleLocalShot = handleLocalShot
...
local AIMBOT_RENDER_BIND = "zaiAimbotUpdate"
...
local fonts = {}
fonts.main = getgenv().zaiUIFont or Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Black)
...
local function fortniteffect(adornee, color, damageText)
    ...
    bb.Size = UDim2.new(0, math.floor(70 * Library.MobileScale), 0, math.floor(40 * Library.MobileScale))
...
hitSoundsTab:AddToggle("DisableGunSounds", {
    ...
    Callback = function(val)
        ...
        if val then
            if getgenv().zaiRefreshGunSoundMute then
                getgenv().zaiRefreshGunSoundMute()
            end
        else
...
local function clearVmPartHighlight(part)
    ...
    local legacy = part and part:FindFirstChild("zaiVmPartHighlight")
...
local applyChams
local chamLoop
local CHAM_RENDER_BIND = "zaiViewModelChams"
...
getgenv().zaiUpdViewModelChams = updViewModelEffectLoop

applyChams = function(dt)
    if iszaiMenuOpen and iszaiMenuOpen() then
        return
    end
...
local function viewportloop()
    ...
    viewportLoop = game:GetService("RunService").RenderStepped:Connect(function()
        if iszaiMenuOpen and iszaiMenuOpen() then return end
...
local function bindViewModelAnimator(animator)
    ...
    if animator:GetAttribute("zaiNoAnimBound") then
        return
    end
    animator:SetAttribute("zaiNoAnimBound", true)
...
local function hookViewModelAnimations()
    ...
    if fp then
        ...
        if not vm:GetAttribute("zaiNoAnimVmHooked") then
            vm:SetAttribute("zaiNoAnimVmHooked", true)
...
local function stopNoAnimLoop()
    ...
    local vm = Workspace:FindFirstChild("ViewModels")
    if vm then
        vm:SetAttribute("zaiNoAnimVmHooked", nil)
        for _, desc in ipairs(vm:GetDescendants()) do
            if desc:IsA("Animator") then
                desc:SetAttribute("zaiNoAnimBound", nil)
            end
        end
    end
end
...
local modules = {
    enums = zaiSafeRequire(replicatedstorage.Modules.EnumLibrary),
    fighter = zaiSafeRequire(player.PlayerScripts.Controllers.FighterController),
    camcontrol = zaiSafeRequire(player.PlayerScripts.Controllers.CameraController),
    utility = zaiSafeRequire(replicatedstorage.Modules.Utility)
}

local MATCH_ID_ATTRS = {"MatchId", "DuelId", "RoundId", "GameId", "MatchUUID", "ArenaId", "zaiId"}
...
getgenv().zaiIsInActiveMatch = liveMatch
getgenv().zaiIsInLobby = lobbyCache
...
local function getammo()
    ...
    local success, controller = pcall(function()
        return zaiSafeRequire(player.PlayerScripts.Controllers.FighterController)
    end)
    ...
    for _, targetplayer in players:GetPlayers() do
        if targetplayer ~= player and targetplayer.Character then
            ...
            if valid(char) then
                local root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    local wts = getgenv().zaiWorldToScreen or worldToScreen
...
local function stopsync()
    ...
    clrVoidSnap()
    config.voidspam.phase = nil
    if config.orbit.savedpos and localfighter and localfighter.Entity and localfighter.Entity.RootPart and not getgenv().zaiUndergroundEnabled then
...
local function startsync()
    ...
    config.orbit.connection = runsvc.Heartbeat:Connect(function(dt)
        ...
        if not getgenv().zaiUndergroundEnabled then
            if localfighter and localfighter.Entity and localfighter.Entity.RootPart then
                local currentPos = localfighter.Entity.RootPart.CFrame
...
modules.camcontrol.Update = function(...)
    if getgenv().zaiUndergroundEnabled then
        return oldcamupdate(...)
...
local function updatesling()
    ...
    if slingon then
        ...
        if getgenv().zaiSetUnderground then
            getgenv().zaiSetUnderground(true)
        end
        ...
        if not Toggles.AntiAimUnderground.Value then
            if getgenv().zaiSetUnderground then
                getgenv().zaiSetUnderground(false)
            end
        end
        vhState.active = false
    end
...
runsvc.RenderStepped:Connect(function()
    local inLobby = getgenv().zaiIsInLobby and getgenv().zaiIsInLobby()
...
local function mkStatusLbl()
    local label = getgenv().zaiTrackDrawingText(Drawing.new("Text"))
    label.Size = 13
    label.Font = 2
    label.Outline = true
    label.Center = true
...
local function shouldNotifyPlayerHit(plr)
    ...
    local lastShot = getgenv().zaiCombatLastShotAt or 0
...
local function pollHitNotifHealth()
    ...
    for _, plr in ipairs(players:GetPlayers()) do
        ...
        if cur < last - 0.01 then
            ...
            ragePerf.lastHitAtByPlayer[plr] = tick()
            if cur <= 0 then
                local tryKill = getgenv().zaiTryKillSound
                if tryKill then
...
local function bindHitNotifCharacter(plr, char)
    ...
    hitNotifHumConn[plr] = hum.HealthChanged:Connect(function(newHp)
        ...
        if newHp <= 0 and localHit and localHit.plr and localHit.plr ~= player and not config.hitNotifications.enabled then
            local tryKill2 = getgenv().zaiTryKillSound
            if tryKill2 then
...
v2:AddToggle("cframespf_enabled", {
    ...
    Callback = function(state)
        ...
        if getgenv().zaiConfigLoading then return end
    end
...
v2:AddToggle("cframefly_enabled", {
    ...
    Callback = function(state)
        ...
        if getgenv().zaiConfigLoading then
            return
        end
        if not state then
            cleanupfly()
...
getgenv().zaiCleanupMovement = function()
    cleanupfly()
    _G.keyheldcframe = false
    _G.keyheldcframefly = false
    local char = LP.Character
...
local fighter_controller = zaiSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.FighterController)
local local_fighter = fighter_controller.LocalFighter
local underground_enabled = false
local camera_controller = zaiSafeRequire(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.CameraController)
...
local function updantiaim(deltatime)
    ...
    if getgenv().zaiConfigLoading then return end
    if getgenv().zaiConfigLoading == nil then return end
...
getgenv().zaiFlushMovementState = function()
    flushAntiAimMovementState()
    if getgenv().zaiCleanupMovement then
        pcall(getgenv().zaiCleanupMovement)
    else
        _G.keyheldcframe = false
        _G.keyheldcframefly = false
    end
...
getgenv().zaiSyncAfterConfigLoad = function()
    if getgenv().zaiFlushMovementState then
        pcall(getgenv().zaiFlushMovementState)
    end
    ...
    if Toggles and Toggles.AntiAimUnderground then
        ...
        getgenv().zaiUndergroundEnabled = underground_enabled
        if not underground_enabled then
            underground_oldpos = nil
        end
    else
        underground_enabled = false
        getgenv().zaiUndergroundEnabled = false
        underground_oldpos = nil
    end
...
antiaimbox:AddToggle("AntiAimUnderground", {
    ...
    Callback = function(value)
        underground_enabled = value
            getgenv().zaiUndergroundEnabled = value
            if getgenv().zaiConfigLoading then return end
            if not value then
            underground_oldpos = nil
        end
...
getgenv().zaiSetUnderground = function(val)
    underground_enabled = val
    getgenv().zaiUndergroundEnabled = val
    if not val then
        underground_oldpos = nil
    end
end
...
local getAspectStretch = getgenv().zaiGetAspectStretch
local worldToEspScreen = getgenv().zaiWorldToScreenEsp or getgenv().zaiWorldToScreen
local espScreenAnchor = getgenv().zaiScreenAnchor
...
local function makelabel(parent)
    local label = Instance.new("TextLabel")
    label.Parent = parent
    label.Size = UDim2.new(0, math.floor(200 * Library.MobileScale), 0, math.floor(20 * Library.MobileScale))
    label.BackgroundTransparency = 1
    label.TextColor3 = Color3.fromRGB(255, 255, 255)
...
local ESP_RENDER_BIND = "zaiESPUpdate"
...
local function trackUtilDrawingText(obj)
    if getgenv().zaiTrackDrawingText then
        return getgenv().zaiTrackDrawingText(obj)
    end
    return obj
end
...
local function makedrawtext(text, color)
    ...
    t.Center = true
    t.Font = getgenv().zaiHudDrawFont or 2
    return t
end
            ...
            if onScreen and utilconfig.enable and isselected(realname) then
                ...
                if distdraw then
                    if utilconfig.distance then
                        ...
                        distdraw.Text = getgenv().zaiL("studs_short", dist)
                        if showImg then
...
local function trackBloomEffect(bloom)
    if not bloom or not bloom:IsA("BloomEffect") or bloom.Name == "zaiExtraBloom" then
        return
    end
...
local function applyBloomModifier()
    ...
    for _, child in ipairs(Lighting:GetChildren()) do
        if child:IsA("BloomEffect") and child.Name ~= "zaiExtraBloom" then
            child.Enabled = false
        end
    end

    if bloomCfg.Enabled then
        local extraBloom = Lighting:FindFirstChild("zaiExtraBloom")
        if not extraBloom then
            extraBloom = Instance.new("BloomEffect")
            extraBloom.Name = "zaiExtraBloom"
        ...
        local legacyExtra = Lighting:FindFirstChild("zaiExtraBloom")
        if legacyExtra then
            legacyExtra:Destroy()
        end
        for _, child in ipairs(Lighting:GetChildren()) do
...
local function ensureExtraEffects()
    ...
    if ccCfg and ccCfg.Enabled then
        if not extraColorCorrection or not extraColorCorrection.Parent then
            ...
            extraColorCorrection.Name = "zaiExtraColorCorrection"
...
local WATERMARK_TEXT = "zai"

local textChars = {}
for i = 1, #WATERMARK_TEXT do
    local char = getgenv().zaiTrackDrawingText(Drawing.new("Text"))
    char.Size = 13
    char.Font = 2
    char.Outline = true
...
local ammoLabel = getgenv().zaiTrackDrawingText(Drawing.new("Text"))
ammoLabel.Size = 13
ammoLabel.Font = 2
...
getgenv().zaiPlayKillSound = playKillSound

getgenv().zaiTryKillSound = function(plr, lastHp, newHp)
    if not ksCfg.Enabled then
        return
    end
    if not plr or plr == ksLocalPlayer then
...
local ASPECT_RATIO_BIND = "zaiAspectRatio"
...
getgenv().zaiStopSkyboxUpdater = stopSkyboxUpdater
...
game:GetService("ScriptContext").DescendantRemoving:Connect(function(descendant)
    if descendant == script then
        local stopSkyboxUpdater = getgenv().zaiStopSkyboxUpdater
...
Library:OnUnload(function()
    ...
    getgenv().zaiKeybindList = nil
    Library.Unloaded = true
end)

Library.KeybindFrame.Visible = false

getgenv().zaiKeybindList = getgenv().zaiKeybindList or {}
local kbList = getgenv().zaiKeybindList
local KB_ROW_H = 18
local KB_PAD = 8
local KB_TITLE_H = 28
...
local function kbTextWidth(text, textSize)
    ...
    local ok, sz = pcall(function()
        local fontFace = getgenv().zaiGetUiFont and getgenv().zaiGetUiFont()
...
local function buildKbListUi()
    ...
    getgenv().zaiApplyUiFont(title, 14)
    ...
    scroll.CanvasSize = UDim2.fromOffset(math.floor(0 * Library.MobileScale), math.floor(0 * Library.MobileScale))
...
applyKbListTheme = function()
    ...
    if kbList.title then
        ...
        getgenv().zaiApplyUiFont(kbList.title, 14)
    end
    ...
    for _, row in ipairs(kbList.rowPool) do
        if row.lbl then
            ...
            getgenv().zaiApplyUiFont(row.lbl, 12)
        end
        if row.frame then
...
getgenv().zaiApplyKbListTheme = function()
    kbThemeDirty = true
    applyKbListTheme()
end
getgenv().zaiRefreshKbList = function(force)
...
do
    local prevSync = getgenv().zaiSyncAfterConfigLoad
    getgenv().zaiSyncAfterConfigLoad = function()
...
local function syncMenuOverlayUi()
    ...
    if open and getgenv().zaiCosmeticUIState and getgenv().zaiCosmeticUIState.applyLiveViewModelUpdate then
        task.defer(function()
            pcall(getgenv().zaiCosmeticUIState.applyLiveViewModelUpdate)
        end)
    end
    if open and getgenv().zaiApplyCosmeticUiTheme then
        pcall(getgenv().zaiApplyCosmeticUiTheme)
    end
end
...
MenuGroup:AddDropdown('zaiLanguage', {
    Text = 'language',
    Values = { 'english', 'korean', 'spanish' },
    Default = getgenv().zaiLanguage or 'english',
    Callback = function(val)
        getgenv().zaiLanguage = val
        if getgenv().zaiReloadLanguageFont then
            getgenv().zaiReloadLanguageFont(val)
        end
        if getgenv().zaiApplyUIFont then
            getgenv().zaiApplyUIFont()
        end
        if getgenv().zaiRefreshLocaleUI then
            pcall(getgenv().zaiRefreshLocaleUI)
        end
    end,
})
...
do
    local oldBlur = Lighting:FindFirstChild("zaiTextureBlur")
    if oldBlur then
        oldBlur:Destroy()
    end
end
...
getgenv().zaiUpdSmoothTextures = updSmoothTextures
getgenv().zaiUpdDarkTextures = updDarkTextures
getgenv().zaiUpdTransparentTextures = updTransparentTextures
...
texturesBox:AddToggle('smooth_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().zaiUpdSmoothTextures
        if fn then fn() end
    end
})
...
texturesBox:AddToggle('dark_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().zaiUpdDarkTextures
        if fn then fn() end
    end
})
...
local transparentToggle = texturesBox:AddToggle('transparent_textures', {
    ...
    Callback = function(val)
        ...
        local fn = getgenv().zaiUpdTransparentTextures
        if fn then fn() end
    end
})
...
function ThemeManager:ThemeUpdate()
    ...
    if getgenv().zaiApplyAccentTheme then
        pcall(getgenv().zaiApplyAccentTheme)
    end
    if applyTargetHudTheme then
        applyTargetHudTheme()
    end
    if getgenv().zaiApplyCosmeticUiTheme then
        pcall(getgenv().zaiApplyCosmeticUiTheme)
    end
    if getgenv().zaiKeybindList and getgenv().zaiKeybindList.inner then
        if getgenv().zaiApplyKbListTheme then
            pcall(getgenv().zaiApplyKbListTheme)
        end
    end
end
...
ThemeManager:SetFolder('zai')
SaveManager:SetFolder('zai/rivals')
SaveManager:BuildConfigSection(Tabs['UI Settings'])

getgenv().zaiConfigLoading = true
applyZaiAccentTheme()

task.defer(function()
    task.wait(0.35)
    pcall(applyZaiAccentTheme)
    pcall(function()
    ...
    task.wait(1.25)
    pcall(applyZaiAccentTheme)
    getgenv().zaiConfigLoading = false
end)
end)()